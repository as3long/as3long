 ...
 */
class org.weemvc.as2.PaperLogger {
	/** @private **/
	static private var m_instance:PaperLogger = null;
	/** @private **/
	//这里 logger 是静态类
	private var m_traceLogger:Object;
	
	/**
	 * 日志输出类构造函数。
	 * 
	 * @internal	注意：throws 标签 tab 不友好，所以不能用 tab 来做分隔
	 * @throws org.weemvc.as2.WeemvcError 单件的<code>PaperLogger</code>被实例化多次
	 */
	 ...
	 */
	public function addLogger(logger:Object):Void {
	 ...
	 * 
	 * @param	info		当前要输出的信息
	 * @param	className	当前输出内容的类名
	 * @param	rest		其他要输出的任何信息
	 */
	public function log(info:String, className:String, rest:Array):Void {
		if (m_traceLogger) {
			m_traceLogger.record(WeemvcError.formatMessage("WeeMVC Warning# ", info, className, rest));
		}else {
			trace(WeemvcError.formatMessage("WeeMVC Warning# ", info, className, rest));
		}
	}
}